...
public class ItemManager {
     ...
     *  @param fileName  文件名
     * @param tableHeader  上层提供的表头信息
     * @return 数据项管理器
     * @throws FileExistException 想新建的文件已经存在的异常
     */
    public static IItemStorage createFile(String fileName, byte[] tableHeader) throws FileExistException {
        // 如果文件已经存在，那么就抛出文件已存在异常
        if (maps.containsKey(fileName)){
            throw new FileExistException(1);
        }else {
            // 若文件不存在，则创建文件。
            IItemStorage iItemStorage = MockItemStorage.ofNewFile(fileName,tableHeader);;
            maps.put(fileName,iItemStorage);
            return iItemStorage;
        }
